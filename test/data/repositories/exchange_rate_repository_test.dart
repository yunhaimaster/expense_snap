  ...
  group('ExchangeRateRepository', () {
    group('getRate', () {
      test('HKD 固定回傳 1:1 匯率', () async {
        ...
        result.fold(
          ...
          onSuccess: (info) {
            expect(info.rate, CurrencyConstants.ratePrecision);
      ...
      test('快取有效時直接回傳', () async {
        ...
        when(
          mockDb.getExchangeRateCache('CNY', baseCurrency: 'HKD'),
        ...
        result.fold(
          onFailure: (e) => fail('Should succeed'),
          onSuccess: (info) {
            expect(info.rate, 1100000);
            expect(info.source, ExchangeRateSource.auto);
          },
        );
        verifyNever(mockApi.fetchRates(baseCurrency: anyNamed('baseCurrency')));
      });
      ...
      test('快取過期時從 API 取得新匯率', () async {
        ...
        when(
          mockApi.fetchRates(baseCurrency: 'HKD'),
        ...
        verify(mockApi.fetchRates(baseCurrency: 'HKD')).called(1);
      });
      ...
      test('API 失敗時使用過期快取（offline）', () async {
        ...
        when(mockApi.fetchRates(baseCurrency: 'HKD')).thenAnswer(
      ...
      test('API 失敗且無快取時使用預設匯率', () async {
        ...
        result.fold(
          onFailure: (e) => fail('Should succeed'),
          onSuccess: (info) {
            expect(info.rate, CurrencyConstants.defaultRatesToHkd['CNY']);
      ...
      test('成功重新整理時更新快取', () async {
        ...
        result.fold(
          ...
          onSuccess: (info) {
            ...
            expect(info['CNY']!.rate, 1100000);
            expect(info['USD']!.rate, 7850000);
          },
        );
    ...
    group('forceRefresh', () {
      test('forceRefresh 繞過冷卻期', () async {
        ...
        expect(result.isSuccess, true);
        // API 被呼叫兩次
        verify(mockApi.fetchRates(baseCurrency: 'HKD')).called(2);
      });
    ...
    group('getAllRates', () {
      test('回傳所有支援幣種的匯率', () async {
        ...
        for (final currency in CurrencyConstants.supportedCurrencies) {
          ...
          when(
            mockDb.getExchangeRateCache(currency, baseCurrency: 'HKD'),
          ).thenAnswer((_) async => null);
        }
